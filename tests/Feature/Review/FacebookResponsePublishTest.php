...
test('it publishes response to correct Facebook page when location has facebook_page_id', function () {
    ...
    // Mock Facebook API - POST /{page-id}/ratings
    Http::fake([
        '*/page_123/ratings*' => Http::response(['success' => true], 200),
    ]);
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), 'page_123/ratings');
    });
});
...
test('it uses correct credential when multiple Facebook pages exist', function () {
    ...
    Http::assertSent(function ($request) {
        return str_contains($request->url(), 'page_A/ratings');
    });

    Http::assertNotSent(function ($request) {
        return str_contains($request->url(), 'page_B/ratings');
    });
});
